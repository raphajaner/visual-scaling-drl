"""Customized openrlbenchmark plotting engine for fetching W&B runs and computing IQM metrics."""
import copy
import os
import scipy
from collections import defaultdict
from dataclasses import dataclass, field
from typing import Dict, List, Literal, Optional
from urllib.parse import parse_qs, urlparse

import expt
import matplotlib.pyplot as plt

plt.rcParams['text.usetex'] = True
plt.rcParams['font.family'] = 'serif'  # This ensures that LaTeX will use the serif family (Latin Modern)
plt.rcParams['text.latex.preamble'] = r'\usepackage{lmodern, amsmath}'

# to reduce the size of the pdf output by using a more aggressive compression level and using TrueType fonts
plt.rcParams['pdf.compression'] = 9  # 0 to 9; 6 is a good balance
plt.rcParams['pdf.fonttype'] = 42  # Use TrueType fonts to prevent bloat/rendering issues

import numpy as np
import pandas as pd
import peewee as pw
import seaborn as sns
import tyro
import wandb
import wandb.apis.reports as wb  # noqa
from dotmap import DotMap
from expt import Hypothesis, Run
from numpy.core.defchararray import title
from rich.console import Console
from rich.pretty import pprint
from rich.table import Table
from rliable import library as rly
from rliable import metrics
from plot_utils import plot_utils
from matplotlib.ticker import MultipleLocator

import openrlbenchmark
import openrlbenchmark.cache
from openrlbenchmark.hns import atari_human_normalized_scores as atari_hns
from openrlbenchmark.offline_db import OfflineRun, OfflineRunTag, Tag, database_proxy

from visual_scaling_drl.eval.normalized_score_lists import progcen_easy_hns, progcen_hard_hns

colors_flatten_original = [c for item in ["colorblind", "bright", ] for c in sns.color_palette(item).as_hex()]

# delete first two since they are used as default
del colors_flatten_original[6]
impala_default_color = colors_flatten_original[1]
impoola_default_color = colors_flatten_original[0]
del colors_flatten_original[1]
del colors_flatten_original[0]


# del colors_flatten_original[6]

@dataclass
class RliableConfig:
    nsubsamples: int = 20
    """the number of subsamples for rliable"""
    score_normalization_method: Literal["maxmin", "atari", "onezero", "100zero", "procgen", "none"] = "maxmin"
    """the method to normalize the scores"""
    normalized_score_threshold: float = 8.0
    """the threshold for the normalized score for the performance profile"""
    normalized_score_threshold_min: Optional[float] = None
    """the minimum threshold for the normalized score for the performance profile"""
    sample_efficiency_and_walltime_efficiency_method: Optional[
        Literal["Median", "IQM", "Mean", "Optimality Gap"]] = "IQM"
    """the method to compute the sample efficiency and walltime efficiency"""
    combined_figure: bool = False
    """if toggled, we will generate the cover figure with IQM"""
    performance_profile_plots: bool = False
    """if toggled, we will generate performance profile plots"""
    aggregate_metrics_plots: bool = True
    """if toggled, we will generate aggregate metrics plots"""
    sample_efficiency_plots: bool = False
    """if toggled, we will generate sample efficiency plots"""
    plot_probability_of_improvement: bool = False
    """if toggled, we will plot the probability of improvement"""
    sample_efficiency_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the sample efficiency"""
    performance_profile_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the performance profile"""
    interval_estimates_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the the interval estimates"""
    aggregate_fig_title: str = None
    """the title of the aggregate figure"""
    overlay_train_test: bool = False
    """plot training/test aggregate metrics together when enabled"""
    overlay_alpha: float = 0.15
    overlays_share_color: bool = True
    overlay_linewidth: float = 1.5
    aggregate_fn: str = "iqm"


@dataclass
class PlotConfig:
    ncols: int = 2
    """the number of columns in the chart"""
    nrows: tyro.conf.Suppress[int] = None
    """(TO BE FILLED in runtime) the number of rows in the chart"""
    ncols_legend: int = 2
    """the number of legend columns in the chart"""
    xlabel: str = "Steps"
    """the label of the x-axis"""
    ylabel: str = "Episodic Return"
    """the label of the y-axis"""
    sharex: bool = False
    """if toggled, we will share the x-axis across all subplots"""
    rolling: int = 100
    """the rolling window for smoothing the curves"""
    time_unit: str = "m"
    """the unit of time in the x-axis of the chart (e.g., `s` for seconds, `m` for minutes, `h` for hours); default: `m`"""
    cm: float = 3.5
    """the multiplier for the column width"""
    rm: float = 3.0
    """the multiplier for the row height"""
    hspace: float = None
    """the height space between subplots"""
    wspace: float = None
    """the width space between subplots"""
    row_split_after: int = None
    """the number of rows after which to add a vertical line"""
    """the number of rows after which to add a vertical line"""


@dataclass
class Args:
    filters: tyro.conf.UseAppendAction[List[List[str]]]
    """the filters of the experiments; see docs"""
    env_ids: tyro.conf.UseAppendAction[List[List[str]]]
    """the ids of the environment to compare"""
    output_filename: str = "compare"
    """the output filename of the plot, without extension"""
    metric_last_n_average_window: int = 100
    """the last n number of episodes to average metric over in the result table"""
    scan_history: bool = False
    """if toggled, we will pull the complete metrics from wandb instead of sampling 500 data points (recommended for generating tables)"""
    check_empty_runs: bool = True
    """if toggled, we will check for empty wandb runs"""
    report: bool = False
    """if toggled, a wandb report will be created"""
    wandb_project_name: str = "cleanrl"
    """the wandb project name for the report creation"""
    offline: bool = False
    """if toggled, we will use the offline database instead of wandb"""
    pc: PlotConfig = field(default_factory=PlotConfig)
    """the plot configuration"""
    rliable: bool = False
    """if toggled, we will use rliable to compute the metrics"""
    rc: RliableConfig = field(default_factory=RliableConfig)
    """the rliable configuration"""


class Runset:
    def __init__(
            self,
            name: str,
            entity: str,
            project: str,
            metric: str = "charts/episodic_return",
            groupby: str = "",
            custom_exp_name_key: str = "exp_name",
            exp_name: str = "",
            custom_env_id_key: str = "env_id",
            env_id: str = "",
            tags: List[str] = [],
            username: str = "",
            color: str = "#000000",
            marker: str = "o",
            linestyle: str = "solid",
            offline_db: pw.Database = None,
            offline: bool = False,
            seeds: List[int] = []
    ):
        self.name = name
        self.entity = entity
        self.project = project
        self.metric = metric
        self.groupby = groupby
        self.custom_exp_name_key = custom_exp_name_key
        self.exp_name = exp_name
        self.custom_env_id_key = custom_env_id_key
        self.env_id = env_id
        self.color = color
        self.marker = marker
        self.linestyle = linestyle
        self.tags = tags
        self.username = username
        self.offline_db = offline_db
        self.offline = offline
        self.seeds = seeds

        user = [{"username": self.username}] if self.username else []
        include_tag_groups = [{"tags": {"$in": [tag]}} for tag in self.tags] if len(self.tags) > 0 else []
        include_seeds_groups = [{"config.seed.value": {"$in": self.seeds}}] if len(self.seeds) > 0 else []
        self.wandb_filters = {
            "$and": [
                {f"config.{self.custom_env_id_key}.value": self.env_id},
                *include_tag_groups,
                *user,
                *include_seeds_groups,
                {f"config.{self.custom_exp_name_key}.value": self.exp_name},
            ]
        }

    @property
    def runs(self):
        if not self.offline:
            return wandb.Api().runs(
                path=f"{self.entity}/{self.project}",
                filters=self.wandb_filters,
            )
        else:
            with self.offline_db.bind_ctx([OfflineRun, OfflineRunTag, Tag]):
                cond = (
                        (OfflineRun.project == self.project)
                        & (OfflineRun.entity == self.entity)
                        & (OfflineRun.config[self.custom_env_id_key] == self.env_id)
                        & (OfflineRun.config[self.custom_exp_name_key] == self.exp_name)
                )
                if self.username:
                    cond = cond and OfflineRun.username == self.username
                if len(self.tags) > 0:
                    for tag_str in self.tags:
                        cond = cond & (Tag.name == tag_str)
                    query = OfflineRun.select().join(OfflineRunTag).join(Tag).where(cond)
                else:
                    query = OfflineRun.select().where(cond)
                g = [run for run in query]

            return g

    @property
    def report_runset(self):
        return wb.Runset(
            name=self.name,
            entity=self.entity,
            project=self.project,
            filters={"$or": [self.wandb_filters]},
            groupby=[self.groupby] if len(self.groupby) > 0 else None,
        )


def print_rich_table(title: str, df: pd.DataFrame, console: Console) -> Table:
    table = Table()
    for column in df.columns:
        table.add_column(column)
    for _, row in df.iterrows():
        table.add_row(*row.astype(str).tolist())
    console.rule(f"[bold red]{title}")
    console.print(table)


def create_hypothesis(runset: Runset, scan_history: bool = False) -> Hypothesis:
    runs = []
    for idx, run in enumerate(runset.runs):
        print("loading", run, run.url)
        if run.state == "running":
            print(f"Skipping running run: {run}")
            continue

        if scan_history:
            run = openrlbenchmark.cache.CachedRun(run, cache_dir=os.path.join(openrlbenchmark.__path__[0], "dataset"))
            with runset.offline_db.bind_ctx([OfflineRun, OfflineRunTag, Tag]):
                tags = []
                for tag_str in run.run.tags:
                    tag = Tag.get_or_none(name=tag_str)
                    if not tag:
                        tag = Tag.create(name=tag_str)
                        tag.save()
                    tags.append(tag)
                offline_run = OfflineRun.get_or_none(id=run.run.id)
                if offline_run:
                    offline_run.delete_instance()
                offline_run = OfflineRun.create(
                    id=run.run.id,
                    name=run.run.name,
                    state=run.run.state,
                    url=run.run.url,
                    path=run.run.path,
                    username=run.run.user.username,
                    tags=tags,
                    project=run.run.project,
                    entity=run.run.entity,
                    config=run.run.config.toDict() if isinstance(run.run.config, DotMap) else run.run.config,
                )
                offline_run.save()
            run_df = run.run_df
        else:
            # run_df = run.history(samples=1500)
            history_keys = ["global_step", "_runtime", runset.metric]

            run_df = run.history(samples=10000000, keys=history_keys)
            if args.rc.overlay_train_test:
                # replace the test with train in the metric key
                overlay_train_key = runset.metric.replace("test", "train")
                run_df1 = run.history(samples=10000000, keys=[overlay_train_key])
                # combine the two dataframes
                run_df = pd.merge(run_df, run_df1, left_index=True, right_index=True, how='outer')

        if "videos" in run_df:
            run_df = run_df.drop(columns=["videos"], axis=1)
        if len(runset.metric) > 0:
            overlay_columns = {}
            try:
                # normalize the metric given the ranges for the environment
                env_id = runset.env_id
                if "distribution_mode" in runset.runs[0].config:
                    if runset.runs[0].config["distribution_mode"] == "easy":
                        procgen_hns = progcen_easy_hns
                    else:
                        procgen_hns = progcen_hard_hns

                # # TODO: make normalization depending on the rc setting
                # import pdb
                # pdb.set_trace()

                if env_id in procgen_hns and 'scores' in runset.metric:
                    run_df["charts/episodic_return"] = (run_df[runset.metric] - procgen_hns[env_id][1]) / (
                            procgen_hns[env_id][2] - procgen_hns[env_id][1])
                    if args.rc.overlay_train_test:
                        run_df["charts/episodic_return_overlay"] = (run_df[overlay_train_key] - procgen_hns[env_id][
                            1]) / (
                                                                           procgen_hns[env_id][2] - procgen_hns[env_id][
                                                                       1])
                        overlay_columns["charts/episodic_return_overlay"] = "charts/episodic_return_overlay"
                else:
                    run_df["charts/episodic_return"] = run_df[runset.metric]
                    if args.rc.overlay_train_test:
                        run_df["charts/episodic_return_overlay"] = run_df[overlay_train_key]
                        overlay_columns["charts/episodic_return_overlay"] = "charts/episodic_return_overlay"

            except KeyError:
                import pdb
                pdb.set_trace()
        # run_df["charts/episodic_return"] = run_df["charts/episodic_return"].max()
        cleaned_columns = ["global_step", "_runtime", "charts/episodic_return"]
        cleaned_columns.extend(overlay_columns.values())
        cleaned_df = run_df[cleaned_columns].dropna()

        runs += [Run(f"seed{idx}", cleaned_df)]
    return Hypothesis(runset.name, runs)


def compare(
        console: Console,
        runsetss: List[List[Runset]],
        env_ids: List[str],
        metric_last_n_average_window: int,
        scan_history: bool = False,
        output_filename: str = "compare",
        report: bool = False,
        pc: PlotConfig = None,
):
    blocks = []
    if report:
        for idx, env_id in enumerate(env_ids):
            metric_over_step = wb.LinePlot(
                x="global_step",
                y=list({runsets[idx].metric for runsets in runsetss}),
                title=env_id,
                title_x="Steps",
                title_y="Episodic Return",
                max_runs_to_show=100,
                smoothing_factor=0.8,
                groupby_rangefunc="stderr",
                legend_template="${runsetName}",
            )
            metric_over_step.config["aggregateMetrics"] = True
            metric_over_time = wb.LinePlot(
                x="_runtime",
                y=list({runsets[idx].metric for runsets in runsetss}),
                title=env_id,
                title_y="Episodic Return",
                max_runs_to_show=100,
                smoothing_factor=0.8,
                groupby_rangefunc="stderr",
                legend_template="${runsetName}",
            )
            metric_over_time.config["aggregateMetrics"] = True
            pg = wb.PanelGrid(
                runsets=[runsets[idx].report_runset for runsets in runsetss],
                panels=[
                    metric_over_step,
                    metric_over_time,
                    # wb.MediaBrowser(
                    #     num_columns=2,
                    #     media_keys="videos",
                    # ),
                ],
            )
            custom_run_colors = {}
            for runsets in runsetss:
                custom_run_colors.update(
                    {
                        (
                            runsets[idx].report_runset.name,
                            runsets[idx].runs[0].config[runsets[idx].custom_exp_name_key],
                        ): runsets[idx].color
                    }
                )
            pg.custom_run_colors = custom_run_colors  # IMPORTANT: custom_run_colors is implemented as a custom `setter` that needs to be overwritten unlike regular dictionaries
            blocks += [pg]

    figsize = (pc.ncols * pc.cm, pc.nrows * pc.rm)
    fig, axes = plt.subplots(
        nrows=pc.nrows,
        ncols=pc.ncols,
        figsize=figsize,
        sharex=pc.sharex,
        # sharey=True,
    )
    fig_time, axes_time = plt.subplots(
        nrows=pc.nrows,
        ncols=pc.ncols,
        figsize=figsize,
        sharex=pc.sharex,
        # sharey=True,
    )
    if len(env_ids) == 1:
        axes = np.array([axes])
        axes_time = np.array([axes_time])
    axes_flatten = axes.flatten()
    axes_time_flatten = axes_time.flatten()

    result_table = pd.DataFrame(index=env_ids, columns=[runsets[0].name for runsets in runsetss])
    exs = []
    runtimes = []
    global_steps = []

    scaling_data = []

    for idx, env_id in enumerate(env_ids):
        print(f"collecting runs for {env_id}")
        hypotheses = [create_hypothesis(runsets[idx], scan_history) for runsets in runsetss]
        ex = expt.Experiment("Comparison", hypotheses)
        exs.append(ex)

        # for each run `i` get the average of the last `rolling` episodes as r_i
        # then take the average and std of r_i as the results.
        result = []
        for hypothesis in ex.hypotheses:
            metric_result = []
            console.print(f"{hypothesis.name} has {len(hypothesis.runs)} runs", style="bold")
            for run in hypothesis.runs:
                metric_result += [run.df["charts/episodic_return"].dropna()[-metric_last_n_average_window:].mean()]

                # convert time unit in place
                if pc.time_unit == "m":
                    run.df["_runtime"] /= 60
                elif pc.time_unit == "h":
                    run.df["_runtime"] /= 3600
            metric_result = np.array(metric_result)

            mean_val = metric_result.mean()
            std_val = metric_result.std()
            scaling_data.append({
                'Game': env_id,
                'Name': hypothesis.name,
                'Mean': mean_val,
                'Std': std_val
            })

            result += [f"{mean_val:.2f} ± {std_val:.2f}"]

        result_table.loc[env_id] = result
        runtimes.append(list(ex.summary()["_runtime"]))
        global_steps.append(list(ex.summary()["global_step"]))
        ax = axes_flatten[idx]

        # for runsets in runsetss:
        #     if "Training" in runsets[idx].name:
        #         runsets[idx].linestyle = "dotted"

        # for runsets in runsetss:
        #     if "easy" in runsets[idx].name:
        #         runsets[idx].linestyle = "dotted"

        local_n_cols = pc.ncols_legend

        for runsets in runsetss:
            # if local_n_cols > 2:

            if "Impala" in runsets[idx].name:
                runsets[idx].linestyle = "dotted"
                # standard impala color
                # runsets[idx].color = impala_default_color
                # impalaa should have the same marker as impoola, i.e., replace name and set color
                impoola_same_name = runsets[idx].name.replace("Impala", "Impoola")

                for other_runsets in runsetss:
                    if other_runsets[idx].name == impoola_same_name:
                        other_runsets[idx].marker = runsets[idx].marker
                        other_runsets[idx].color = runsets[idx].color

            # elif "Impoola" in runsets[idx].name:
            #     runsets[idx].linestyle = "solid"
            #     # standard impoola color
            #     runsets[idx].color = impoola_default_color
            # else:
            #     #  default_palette[1] for impala,  default_palette[0] for impoola
            #     if "Impala" in runsets[idx].name:
            #         runsets[idx].color = colors_flatten_original[1]
            #     elif "Impoola" in runsets[idx].name:
            #         runsets[idx].color = colors_flatten_original[0]

        # for runsets in runsetss:
        #     if "GAP" in runsets[idx].name:
        #         runsets[idx].linestyle = "solid"
        #     else:
        #         runsets[idx].linestyle = "dotted"

        #
        # marker = [runsets[idx].marker for runsets in runsetss]
        linestyle = [runsets[idx].linestyle for runsets in runsetss]
        # exp_names = [runsets[idx].exp_name for runsets in runsetss]

        # # update matplotlib.rcParams['axes.prop_cycle'] to hack the linestyle
        plt.rcParams['axes.prop_cycle'] = plt.cycler(linestyle=linestyle)

        ex.plot(
            ax=ax,
            title=env_id,
            x="global_step",
            y="charts/episodic_return",
            err_style="band",
            std_alpha=0.1,
            n_samples=901,
            rolling=pc.rolling,
            colors=[runsets[idx].color for runsets in runsetss],
            legend=False,
            fontsize="small",
            markers=[runsets[idx].marker for runsets in runsetss],
            markevery=100,
            markerfacecolor="none",
            linewidth=1.5,
            markersize=4.0,
            # rasterized=True,  # This keeps text sharp but compresses the heavy data lines
            # make dotted lines for the y-axis
        )

        # set matplotlib.rcParams['axes.prop_cycle'] back to solid
        plt.rcParams['axes.prop_cycle'] = plt.cycler(linestyle=['-'])

        ax.set_xlabel("")
        ax.set_ylabel("")
        # captalize the first letter of the title
        ax.set_title(r'\textbf{' + env_id[0].upper() + env_id[1:] + '}',
                     fontsize='small')  # Insert variable into LaTeX bold formatting
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)

        # set x-axis label for the whole figure
        # ax.set_xlabel("Epochs", fontsize='large')
        # wrect = 10
        # hrect = 10
        # ticklabelsize = 'small'
        # ax.tick_params(length=0.1, width=0.1, labelsize=ticklabelsize)
        # ax.spines['left'].set_position(('outward', hrect))
        # ax.spines['bottom'].set_position(('outward', wrect))

        # ax.yaxis.set_major_locator(MultipleLocator(0.1))  # Y-axis tick steps

        ax_time = axes_time_flatten[idx]
        ex.plot(
            ax=ax_time,
            title=env_id,
            x="_runtime",
            y="charts/episodic_return",
            err_style="band",
            std_alpha=0.1,
            n_samples=91,
            rolling=pc.rolling,
            colors=[runsets[idx].color for runsets in runsetss],
            legend=False,
            fontsize="small",
            # marker=[runsets[idx].marker for runsets in runsetss],
            markevery=10,
            markerfacecolor="none",
            linewidth=1.5,
            markersize=3.0,
            # rasterized=True,  # This keeps text sharp but compresses the heavy data lines
        )
        ax_time.set_xlabel("")
        ax_time.set_ylabel("")
    runtimes = pd.DataFrame(np.array(runtimes), index=env_ids, columns=list(ex.summary()["name"]))
    global_steps = pd.DataFrame(np.array(global_steps), index=env_ids, columns=list(ex.summary()["name"]))
    print_rich_table(f"Runtime ({pc.time_unit}) (mean ± std)", runtimes.rename_axis("Environment").reset_index(),
                     console)

    # create the required directory for `output_filename`
    if len(os.path.dirname(output_filename)) > 0:
        os.makedirs(os.path.dirname(output_filename), exist_ok=True)
    print_rich_table(f"{pc.ylabel} (mean ± std)", result_table.rename_axis("Environment").reset_index(), console)

    result_table.to_markdown(open(f"{output_filename}.md", "w"))

    # result_table.to_csv(open(f"{output_filename}.csv", "w"))

    # runtimes.to_markdown(open(f"{output_filename}_runtimes.md", "w"))
    # runtimes.to_csv(open(f"{output_filename}_runtimes.csv", "w"))

    average_runtime = pd.DataFrame(runtimes.mean(axis=0)).reset_index()
    average_runtime.columns = ["Environment", "Average Runtime"]
    print_rich_table(f"Runtime ({pc.time_unit}) Average", average_runtime, console)

    # Plot scaling trends if applicable
    if len(scaling_data) > 0:
        scaling_df = pd.DataFrame(scaling_data)
        # Check if names look like they have resolutions
        if scaling_df['Name'].astype(str).str.contains(r'\(\d+,').any():
            print("Generating scaling trends plot...")
            plot_utils.plot_scaling_trends(scaling_df, f"{output_filename}_scaling.pdf", pc)

    # change the style of every second plot in the figure to dotted lines
    # they should also have the same color as the solid lines

    # for idx,line in enumerate(axes_flatten[0].get_lines()):
    #     if idx % 2 == 1:
    #         line.set_linestyle("--")
    #         # set the color of the dotted line to the color of the solid line
    #         line.set_color(axes_flatten[0].get_lines()[idx-1].get_color())

    # add legend

    h, l = axes_flatten[0].get_legend_handles_labels()

    if pc.ncols_legend > 1:
        e = len(h)
        # alternate the legend entries between the first half and the second half
        h_new = []
        l_new = []
        n_rows = int(e / local_n_cols)
        for r in range(local_n_cols):
            for c in range(n_rows):
                idx = c * local_n_cols + r
                if idx < e:
                    h_new.append(h[idx])
                    l_new.append(l[idx])
        h = h_new
        l = l_new

    # print the labels
    print(f'Legend labels: {l}')

    fig.legend(
        h, l,
        loc="lower center",
        # loc="upper left",
        ncol=local_n_cols,
        # ncol=pc.ncols_legend * (pc.ncols // 2),
        # ncol=1,
        bbox_to_anchor=(0.5, 0.97),
        # bbox_to_anchor=(0.98, 0.98),
        bbox_transform=fig.transFigure,
        frameon=False,
        fontsize='large'
    )

    fig.supxlabel(pc.xlabel, fontsize='large', y=0.02)
    fig.supylabel(pc.ylabel, fontsize='large', x=0.02)
    fig.tight_layout()

    title_name = args.output_filename.split('/')[-1]
    title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'

    # if args.rc.aggregate_fig_title is not None:
    #     title_name += f' {args.rc.aggregate_fig_title}'
    # # set the overall title of the figure
    # fig.suptitle(title_name, fontsize='large', y=1.1)

    h, l = axes_time_flatten[0].get_legend_handles_labels()

    fig_time.legend(
        h, l, loc="lower center", ncol=pc.ncols_legend, bbox_to_anchor=(0.5, 0.95), bbox_transform=fig_time.transFigure,
        frameon=False
    )
    fig_time.supxlabel(f"Time ({pc.time_unit})")
    fig_time.supylabel(pc.ylabel)
    fig_time.tight_layout()

    # remove the empty axes
    for ax in axes_flatten[len(env_ids):]:
        ax.remove()
    for ax in axes_time_flatten[len(env_ids):]:
        ax.remove()

    print(f"saving figures and tables to {output_filename}")
    fig.subplots_adjust(hspace=pc.hspace, wspace=pc.wspace)
    fig_time.subplots_adjust(hspace=pc.hspace, wspace=pc.wspace)
    # fig.savefig(f"{output_filename}.png", bbox_inches="tight")
    fig.savefig(f"{output_filename}.pdf", bbox_inches="tight",
                dpi=150,
                transparent=True,
                # backend='pgf' if plt.rcParams['text.usetex'] else None,  # PGF is often more efficient for LaTeX
                )

    # fig.savefig(f"{output_filename}.svg", bbox_inches="tight")
    # fig_time.savefig(f"{output_filename}-time.png", bbox_inches="tight")
    # fig_time.savefig(f"{output_filename}-time.pdf", bbox_inches="tight", dpi=150)
    # fig_time.savefig(f"{output_filename}-time.svg", bbox_inches="tight")
    return blocks, runtimes, global_steps, exs


def normalize_score(score_dict: Dict[str, np.ndarray], max_scores: np.ndarray, min_scores: np.ndarray):
    """
    Each item in `score_dict` has shape (num_seeds, num_envs, num_subsamples)
    `max_scores` has shape (num_envs)
    `min_scores` has shape (num_envs)
    """
    normalized_score_dict = {}
    for key in score_dict:
        normalized_score_dict[key] = (score_dict[key] - min_scores.reshape(1, -1, 1)) / (
                max_scores.reshape(1, -1, 1) - min_scores.reshape(1, -1, 1)
        )
    return normalized_score_dict


def maxmin_normalize_score(score_dict: Dict[str, np.ndarray]):
    all_scores = np.concatenate([score_dict[key] for key in score_dict], axis=0)
    max_scores = all_scores.max(0).max(1)  # 1) max over all experiments and seeds 2) max over all steps
    min_scores = all_scores.min(0).min(1)  # 1) min over all experiments and seeds 2) min over all steps
    return normalize_score(score_dict, max_scores, min_scores)


def onezero_normalize_score(score_dict: Dict[str, np.ndarray], upper_value=1):
    all_scores = np.concatenate([score_dict[key] for key in score_dict], axis=0)
    max_scores = all_scores.max(0).max(1)  # 1) max over all experiments and seeds 2) max over all steps
    min_scores = all_scores.min(0).min(1)  # 1) min over all experiments and seeds 2) min over all steps
    one_scores = np.ones_like(max_scores) * upper_value
    zero_scores = np.zeros_like(min_scores)
    return normalize_score(score_dict, one_scores, zero_scores)


def atari_normalize_score(original_env_ids):
    env_ids = []
    for env_id in original_env_ids:
        if env_id.endswith("NoFrameskip-v4"):
            env_id = env_id.replace("NoFrameskip-v4", "-v5")
        env_ids.append(env_id)
    max_scores = np.array([atari_hns[env_id][1] for env_id in env_ids])
    min_scores = np.array([atari_hns[env_id][0] for env_id in env_ids])
    return normalize_score(score_dict, max_scores, min_scores)


def procgen_normalize_score(score_dict: Dict[str, np.ndarray], env_ids):
    max_scores = np.array([procgen_hns[env_id][2] for env_id in env_ids])
    min_scores = np.array([procgen_hns[env_id][1] for env_id in env_ids])
    return normalize_score(score_dict, max_scores, min_scores)


if __name__ == "__main__":
    args = tyro.cli(Args)
    # by default assume all the env_ids are the same
    if len(args.filters) > 1 and len(args.env_ids) == 1:
        args.env_ids = args.env_ids * len(args.filters)
    # calculate the number of rows
    args.pc.nrows = np.ceil(len(args.env_ids[0]) / args.pc.ncols).astype(int)

    console = Console()
    blocks = []
    runsetss = []
    offline_dbs = {}
    # colors_flatten_original = [c for item in ["deep", "dark", "bright", ] for c in sns.color_palette(item).as_hex()]

    # For generalization plot, hack the color and marker cycle
    colors = []
    # for a, b in zip(colors_flatten_original[:-1], colors_flatten_original[1:]):
    #     colors.append(a)
    #     colors.append(b)
    # colors_flatten_original = colors

    # for a in colors_flatten_original:
    #     colors.append(a)
    #     colors.append(a)
    # colors_flatten_original = colors

    markers_flatten_original = \
        ["o", "s", "^", "*", "D", "v", "<", ">", "p", "P", "*", "X", "d", "H", "h", "+", "x", "|", "_"]
    plt.rcParams["axes.prop_cycle"] = plt.cycler("color", colors_flatten_original)
    plt.rcParams["axes.prop_cycle"] = plt.cycler("marker", markers_flatten_original)
    colors_flatten = copy.deepcopy(colors_flatten_original)
    markers_flatten = copy.deepcopy(markers_flatten_original)
    colors = []
    markers = []

    for filters in args.filters:
        colors += [colors_flatten[: len(filters) - 1]]
        colors_flatten = colors_flatten[len(filters) - 1:]
        markers += [markers_flatten[: len(filters) - 1]]
        markers_flatten = markers_flatten[len(filters) - 1:]

    for filters_idx, filters in enumerate(args.filters):
        parse_result = urlparse(filters[0])
        query = parse_qs(parse_result.query)
        metric = query["metric"][0] if "metric" in query else "charts/episodic_return"
        wandb_project_name = query["wpn"][0] if "wpn" in query else args.wandb_project_name
        wandb_entity = query["we"][0] if "we" in query else args.wandb_entity
        custom_env_id_key = query["ceik"][0] if "ceik" in query else "env_id"
        custom_exp_name_key = query["cen"][0] if "cen" in query else "exp_name"
        pprint(
            {
                "wandb_project_name": wandb_project_name,
                "wandb_entity": wandb_entity,
                "custom_env_id_key": custom_env_id_key,
                "custom_exp_name_key": custom_exp_name_key,
                "metric": metric,
            },
            expand_all=True,
        )
        if f"{wandb_entity}/{wandb_project_name}" not in offline_dbs:
            offline_db_folder = os.path.join(openrlbenchmark.__path__[0], "dataset",
                                             f"{wandb_entity}/{wandb_project_name}")
            offline_db_path = os.path.join(offline_db_folder, "offline.sqlite")
            print(offline_db_path)
            os.makedirs(offline_db_folder, exist_ok=True)
            offline_db = pw.SqliteDatabase(offline_db_path)
            database_proxy.initialize(offline_db)
            offline_db.connect()
            offline_db.create_tables([OfflineRun, Tag, OfflineRunTag])
            offline_dbs[f"{wandb_entity}/{wandb_project_name}"] = offline_db

        for filter_str, color, marker in zip(filters[1:], colors[filters_idx], markers[filters_idx]):
            print("=========", filter_str)
            # parse filter string
            parse_result = urlparse(filter_str)
            exp_name = parse_result.path
            query = parse_qs(parse_result.query)
            username = query["user"][0] if "user" in query else None
            tags = query["tag"] if "tag" in query else []
            custom_legend = query["cl"][0] if "cl" in query else ""
            # HACK unescape
            custom_legend = custom_legend.replace("\\n", "\n")
            seeds = [int(q) for q in query["seed"]] if "seed" in query else []

            runsets = []
            for env_id in args.env_ids[filters_idx]:
                runsets.append(
                    Runset(
                        name=f"{wandb_entity}/{wandb_project_name}/{exp_name} ({query})"
                        if custom_legend == ""
                        else custom_legend,
                        entity=wandb_entity,
                        project=wandb_project_name,
                        metric=metric,
                        groupby=custom_exp_name_key,
                        custom_exp_name_key=custom_exp_name_key,
                        exp_name=exp_name,
                        custom_env_id_key=custom_env_id_key,
                        env_id=env_id,
                        tags=tags,
                        username=username,
                        color=color,
                        marker=marker,
                        offline_db=offline_dbs[f"{wandb_entity}/{wandb_project_name}"],
                        offline=args.offline,
                        seeds=seeds,
                    )
                )
                if args.check_empty_runs:
                    console.print(
                        f"{exp_name} [green]({query})[/] in [purple]{env_id}[/] has {len(runsets[-1].runs)} runs")
                    for run in runsets[-1].runs:
                        console.print(f"┣━━ [link={run.url}]{run.name}[/link] with tags = {run.tags}")
                    assert len(runsets[0].runs) > 0, f"{exp_name} ({query}) in {env_id} has no runs"
            runsetss.append(runsets)

    blocks, runtimes, global_steps, exs = compare(
        console,
        runsetss,
        args.env_ids[0],
        output_filename=args.output_filename,
        metric_last_n_average_window=args.metric_last_n_average_window,
        scan_history=args.scan_history,
        report=args.report,
        pc=args.pc,
    )

    if args.rliable:
        # get min num seeds per hypothesis
        min_num_seeds_per_hypothesis = {}
        for runsets in runsetss:
            min_num_seeds_per_hypothesis[runsets[0].name] = float("inf")
        for ex in exs:
            for hypothesis in ex.hypotheses:
                console.print(f"{hypothesis.name} has {len(hypothesis.runs)} runs", style="bold")
                min_num_seeds_per_hypothesis[hypothesis.name] = min(
                    min_num_seeds_per_hypothesis[hypothesis.name], len(hypothesis.runs)
                )

        # create `score_dict`; each item in `score_dict` has shape (num_seeds, len(args.env_ids[0]), nsubsamples)
        score_dict = {}
        step_dict = {}
        max_global_steps = defaultdict(int)
        overlay_score_dict = {}
        overlay_colors = {}
        overlay_pairs = {}
        for runsets_idx, runsets in enumerate(runsetss):
            score_dict[runsets[0].name] = np.zeros(
                (min_num_seeds_per_hypothesis[runsets[0].name], len(args.env_ids[0]), args.rc.nsubsamples + 1)
            )
            step_dict[runsets[0].name] = np.zeros(
                (min_num_seeds_per_hypothesis[runsets[0].name], len(args.env_ids[0]), args.rc.nsubsamples + 1)
            )
            if args.rc.overlay_train_test:
                phase = "train"
                overlay_key = f"{runsets[0].name}:{phase}"
                overlay_score_dict[overlay_key] = np.zeros_like(score_dict[runsets[0].name])
                overlay_pairs.setdefault(runsets[0].name, {})[phase] = overlay_key
                overlay_colors[overlay_key] = runsets[0].color

            # for each seed
            for seed_idx, _ in enumerate(range(min_num_seeds_per_hypothesis[runsets[0].name])):  # exs[0][runsets_idx]
                min_global_step = float("inf")
                print(f"collecting runs for {runsets[0].name} seed idx {seed_idx}")

                runs_of_one_seed = []
                for ex_idx, ex in enumerate(exs):
                    run_of_one_seed = ex[runsets_idx][seed_idx]
                    min_global_step = min(min_global_step, run_of_one_seed.df["global_step"].iloc[-1])
                    runs_of_one_seed.append(run_of_one_seed)

                    # interpolate
                    x_samples = np.linspace(
                        min(run_of_one_seed.df["global_step"]), max(run_of_one_seed.df["global_step"]),
                        num=args.rc.nsubsamples
                    )

                    score_dict[runsets[0].name][seed_idx, ex_idx, 1:] = np.interp(
                        x_samples, run_of_one_seed.df["global_step"], run_of_one_seed.df["charts/episodic_return"]
                    )
                    step_dict[runsets[0].name][seed_idx, ex_idx, 1:] = x_samples
                    if args.rc.overlay_train_test:
                        overlay_score_dict[overlay_pairs[runsets[0].name]["train"]][seed_idx, ex_idx, 1:] = np.interp(
                            x_samples,
                            run_of_one_seed.df["global_step"],
                            run_of_one_seed.df["charts/episodic_return_overlay"],
                        )

                max_global_steps[runsets[0].name] = max(max_global_steps[runsets[0].name], min_global_step)

        exp_names = list(reversed(list(score_dict.keys())))
        colors_flatten = colors_flatten_original
        colors = dict(zip(list(score_dict.keys()), colors_flatten))
        markers_flatten = markers_flatten_original
        markers = dict(zip(list(score_dict.keys()), markers_flatten))
        frames = np.linspace(0, max(max_global_steps.values()), args.rc.nsubsamples)
        print_rich_table(
            f"Items in the `score_dict` used for `rliable`",
            pd.DataFrame(
                data=[score_dict[key].shape for key in score_dict],
                columns=["Number of Seeds", "Number of Environments", "Number of Sub-samples"],
                index=list(score_dict.keys()),
            )
            .rename_axis("Experiments")
            .reset_index(),
            console,
        )

        # normalize scores.
        if args.rc.score_normalization_method == "maxmin":
            normalized_score_dict = maxmin_normalize_score(score_dict)
        elif args.rc.score_normalization_method == "atari":
            normalized_score_dict = atari_normalize_score(args.env_ids[0])
        elif args.rc.score_normalization_method == "onezero":
            normalized_score_dict = onezero_normalize_score(score_dict)
        elif args.rc.score_normalization_method == "100zero":
            normalized_score_dict = onezero_normalize_score(score_dict, 100)
        elif args.rc.score_normalization_method == "procgen":
            normalized_score_dict = procgen_normalize_score(score_dict, args.env_ids[0])
        elif args.rc.score_normalization_method == "none":
            normalized_score_dict = score_dict
        else:
            raise NotImplementedError(f"Normalization method {args.rc.score_normalization_method} not implemented")

        performance_profile_normalized_score_dict = {}
        for key, value in normalized_score_dict.items():
            performance_profile_normalized_score_dict[key] = np.nanmean(value[:, :, -1:], axis=-1)
        overlay_normalized_score_dict = {}

        if args.rc.overlay_train_test:
            overlay_source = normalized_score_dict
            for overlay_key, raw_scores in overlay_score_dict.items():
                overlay_normalized_score_dict[overlay_key] = np.nanmean(raw_scores[:, :, -1:], axis=-1)

        # def avg_iqm(scores):
        #     return np.mean(scipy.stats.trim_mean(scores, proportiontocut=0.25, axis=-1))

        metric_fns = [
            metrics.aggregate_median,
            metrics.aggregate_iqm,
            metrics.aggregate_mean,
            metrics.aggregate_optimality_gap,
        ]
        metric_names = ["Median", "IQM", "Mean", "Optimality Gap"]

        if args.rc.combined_figure:
            print("plotting combined efficiency curve")
            fig_sample_efficiency, axes_sample_efficiency = plt.subplots(
                ncols=1,
                nrows=1,
                figsize=(6 * 1, 3.4 * 1),
                # figsize=(3.5 * 1, 2.8 * 1),
                sharex=args.pc.sharex,
            )

            if args.rc.aggregate_fn == "iqm":
                metric_fn = metrics.aggregate_iqm
                metric_name = "IQM"
            elif args.rc.aggregate_fn == "mean":
                metric_fn = metrics.aggregate_mean
                metric_name = "Mean"
            elif args.rc.aggregate_fn == "median":
                metric_fn = metrics.aggregate_median
                metric_name = "Median"
            else:
                raise NotImplementedError(f"Aggregate function {args.rc.aggregate_fn} not implemented")

            ax = axes_sample_efficiency

            aggregate_fn = lambda scores: np.array(
                [metric_fn(scores[..., frame]) for frame in range(scores.shape[-1])])
            aggregate_scores, aggregate_cis = rly.get_interval_estimates(
                normalized_score_dict, aggregate_fn, reps=args.rc.sample_efficiency_num_bootstrap_reps
            )

            for exp_name in score_dict.keys():
                global_step = global_steps[exp_name].mean()
                # global_step_xaxis = np.linspace(0, global_step, args.rc.nsubsamples)
                global_step_xaxis = step_dict[exp_name][0, 0]

                # make - line when training is contained in exp_name, else make -- line
                if "Testing" in exp_name:
                    linestyles = {exp_name: "-"}
                    # make sure that testing uses the same color as the training line
                    colors[exp_name] = colors[exp_name.replace("Testing", "Training")]
                    markers[exp_name] = markers[exp_name.replace("Testing", "Training")]
                elif "Training" in exp_name:
                    # dotted
                    linestyles = {exp_name: "dotted"}
                else:
                    linestyles = None

                # For dormant neuron plot
                # if "Impala" in exp_name:
                #     linestyles = {exp_name: "dotted"}
                # else:
                #     linestyles = {exp_name: "solid"}
                # global_step_xaxis = global_step_xaxis[2:]
                # aggregate_scores[exp_name] = aggregate_scores[exp_name][2:]
                # aggregate_cis[exp_name] = aggregate_cis[exp_name][:, 2:]

                plot_utils.plot_sample_efficiency_curve(
                    global_step_xaxis,
                    {exp_name: aggregate_scores[exp_name]},
                    {exp_name: aggregate_cis[exp_name]},
                    algorithms=[exp_name],
                    colors=colors,
                    xlabel=r"Steps",
                    ax=ax,
                    # ylabel=metric_name,
                    ylabel=metric_name if args.pc.ylabel is None else args.pc.ylabel + f" ({metric_name})",
                    labelsize="large",
                    ticklabelsize="small",
                    marker=markers,
                    # line style, iterate over the line styles
                    linestyles=linestyles,

                    # markevery=[5, 10, 15, 20, 25],
                    # linewidth=1,
                    # markersize=4,
                )
            # ax.set_yticks(np.linspace(0, args.rc.normalized_score_threshold))
            ax.set_ylim(0, top=args.rc.normalized_score_threshold)

            # get the max globalstep
            max_global_step = max(max_global_steps.values())
            # round to the nearest million
            max_global_step = np.round(max_global_step, -6)

            # ax.set_xticks(np.linspace(5e6, max_global_step, 5))
            ax.set_xticks(np.linspace(0, max_global_step, 11))

            # get title name as the last part of the output_filename that is separated by '/'
            title_name = args.output_filename.split('/')[-1]
            title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'
            # bold title
            if args.rc.aggregate_fig_title is not None:
                title_name += f' {args.rc.aggregate_fig_title}'

            ax.set_title(f"{title_name}", fontsize="large")

            ax.set_xlabel("")
            expt.plot.autoformat_xaxis(ax)

            h, l = ax.get_legend_handles_labels()
            fig_sample_efficiency.legend(
                h,
                l,
                # loc="lower left",
                # loc="lower center",
                loc="center left",
                # ncol=1,
                ncol=args.pc.ncols_legend,
                # bbox_to_anchor=(0.5, 0.95),
                # bbox_to_anchor=(0.05, 0.95),
                bbox_to_anchor=(0.95, 0.5),
                bbox_transform=fig_sample_efficiency.transFigure,
                fontsize="small",
                frameon=False
            )
            # fig_sample_efficiency.supxlabel(args.pc.xlabel, fontsize='large')  #, fontsize="large")
            # set x lavel to args.pc.xlabel
            ax.set_xlabel(args.pc.xlabel, fontsize="large")
            fig_sample_efficiency.tight_layout()
            # fig_sample_efficiency.savefig(f"{args.output_filename}_combined.png", bbox_inches="tight")
            fig_sample_efficiency.savefig(f"{args.output_filename}_combined.pdf", bbox_inches="tight")
            # tikzplotlib.save(f"{args.output_filename}_combined.tex")

    if args.rc.sample_efficiency_plots:
        print("plotting sample efficiency curve (this is slow and may take several minutes)")
        fig_sample_efficiency, axes_sample_efficiency = plt.subplots(
            ncols=2,
            nrows=2,
            figsize=(7 * 2, 3.4 * 2),
            sharex=args.pc.sharex,
        )
        for metric_fn, ax, metric_name in zip(metric_fns, axes_sample_efficiency.flatten(), metric_names):
            aggregate_fn = lambda scores: np.array(
                [metric_fn(scores[..., frame]) for frame in range(scores.shape[-1])])
            aggregate_scores, aggregate_cis = rly.get_interval_estimates(
                normalized_score_dict, aggregate_fn, reps=args.rc.sample_efficiency_num_bootstrap_reps
            )
            for exp_name in score_dict.keys():
                global_step = global_steps[exp_name].mean()
                # global_step_xaxis = np.linspace(0, global_step, args.rc.nsubsamples)
                global_step_xaxis = step_dict[exp_name][0, 0]

                plot_utils.plot_sample_efficiency_curve(
                    global_step_xaxis,
                    {exp_name: aggregate_scores[exp_name]},
                    {exp_name: aggregate_cis[exp_name]},
                    algorithms=[exp_name],
                    colors=colors,
                    xlabel=r"Steps",
                    ax=ax,
                    ylabel=metric_name,
                    labelsize="large",
                    ticklabelsize="large",
                )
            ax.set_xlabel("")
            expt.plot.autoformat_xaxis(ax)

            if metric_name == args.rc.sample_efficiency_and_walltime_efficiency_method:
                fig_median_sample_walltime_efficiency, axes_median_sample_walltime_efficiency = plt.subplots(
                    ncols=2,
                    figsize=(7 * 2, 3.4),
                    sharey=True,
                )
                for exp_name in score_dict.keys():
                    global_step = global_steps[exp_name].mean()
                    # global_step_xaxis = np.linspace(0, global_step, args.rc.nsubsamples)
                    global_step_xaxis = step_dict[exp_name][0, 0]

                    plot_utils.plot_sample_efficiency_curve(
                        global_step_xaxis,
                        {exp_name: aggregate_scores[exp_name]},
                        {exp_name: aggregate_cis[exp_name]},
                        algorithms=[exp_name],
                        colors=colors,
                        xlabel=r"Steps",
                        ax=axes_median_sample_walltime_efficiency[0],
                        ylabel=metric_name,
                        labelsize="large",
                        ticklabelsize="large",
                    )
                expt.plot.autoformat_xaxis(axes_median_sample_walltime_efficiency[0])
                for exp_name in score_dict.keys():
                    runtime = runtimes[exp_name].mean()
                    runtime_xaxis = np.linspace(0, runtime, args.rc.nsubsamples + 1)
                    plot_utils.plot_sample_efficiency_curve(
                        runtime_xaxis,
                        {exp_name: aggregate_scores[exp_name]},
                        {exp_name: aggregate_cis[exp_name]},
                        algorithms=[exp_name],
                        colors=colors,
                        xlabel=f"Time ({args.pc.time_unit})",
                        ax=axes_median_sample_walltime_efficiency[1],
                        ylabel=metric_name,
                        labelsize="large",
                        ticklabelsize="large",
                    )
                axes_median_sample_walltime_efficiency[1].set_ylabel("")
                h, l = axes_median_sample_walltime_efficiency[1].get_legend_handles_labels()
                fig_median_sample_walltime_efficiency.legend(
                    h,
                    l,
                    loc="lower center",
                    ncol=args.pc.ncols_legend,
                    bbox_to_anchor=(0.5, 0.95),
                    bbox_transform=fig_median_sample_walltime_efficiency.transFigure,
                    frameon=False
                )
                fig_median_sample_walltime_efficiency.tight_layout()
                # fig_median_sample_walltime_efficiency.savefig(
                #     f"{args.output_filename}_sample_walltime_efficiency.png", bbox_inches="tight"
                # )
                fig_median_sample_walltime_efficiency.savefig(
                    f"{args.output_filename}_sample_walltime_efficiency.pdf", bbox_inches="tight"
                )

        h, l = axes_sample_efficiency[0][0].get_legend_handles_labels()
        fig_sample_efficiency.legend(
            h,
            l,
            loc="lower center",
            ncol=args.pc.ncols_legend,
            bbox_to_anchor=(0.5, 0.95),
            bbox_transform=fig_sample_efficiency.transFigure,
            frameon=False
        )
        fig_sample_efficiency.supxlabel(args.pc.xlabel, fontsize="large")
        fig_sample_efficiency.tight_layout()
        # fig_sample_efficiency.savefig(f"{args.output_filename}_sample_efficiency.png", bbox_inches="tight")
        fig_sample_efficiency.savefig(f"{args.output_filename}_sample_efficiency.pdf", bbox_inches="tight")

    if args.rc.performance_profile_plots:
        print("plotting performance profiles")
        fig_performance_profile, axes_performance_profile = plt.subplots(
            ncols=2,
            figsize=(7 * 2, 3.4),
        )
        performance_profile_thresholds = np.linspace(0.0, args.rc.normalized_score_threshold, 81)
        score_distributions, score_distributions_cis = rly.create_performance_profile(
            performance_profile_normalized_score_dict,
            performance_profile_thresholds,
            reps=args.rc.performance_profile_num_bootstrap_reps,
        )
        avg_score_distributions, avg_score_distributions_cis = rly.create_performance_profile(
            performance_profile_normalized_score_dict,
            performance_profile_thresholds,
            reps=args.rc.performance_profile_num_bootstrap_reps,
            use_score_distribution=False,
        )
        plot_utils.plot_performance_profiles(
            score_distributions,
            performance_profile_thresholds,
            performance_profile_cis=score_distributions_cis,
            colors=colors,
            xlabel=r"Normalized Score $(\tau)$",
            ax=axes_performance_profile[0],
        )
        plot_utils.plot_performance_profiles(
            avg_score_distributions,
            performance_profile_thresholds,
            performance_profile_cis=avg_score_distributions_cis,
            colors=colors,
            xlabel=r"Normalized Score $(\tau)$",
            ylabel=r"Fraction of tasks with score > $\tau$",
            ax=axes_performance_profile[1],
        )
        h, l = axes_performance_profile[0].get_legend_handles_labels()
        fig_performance_profile.legend(
            h,
            l,
            loc="lower center",
            ncol=args.pc.ncols_legend,
            bbox_to_anchor=(0.5, 0.95),
            bbox_transform=fig_performance_profile.transFigure,
            frameon=False
        )
        fig_performance_profile.tight_layout()
        # fig_performance_profile.savefig(f"{args.output_filename}_performance_profile.png", bbox_inches="tight")
        fig_performance_profile.savefig(f"{args.output_filename}_performance_profile.pdf", bbox_inches="tight")

    if args.rc.plot_probability_of_improvement:
        # Load ProcGen scores as a dictionary containing pairs of normalized score

        # create pairs of the entries in the performance_profile_normalized_score_dict
        compare_score_dict = {}
        for key1 in performance_profile_normalized_score_dict:
            for key2 in performance_profile_normalized_score_dict:
                if key1 != key2:
                    # check if the inverse is not already in the dictionary
                    if key1 + ',' + key2 not in compare_score_dict:
                        combined_key = key2 + ',' + key1
                        if 'Impala,Impala w/ MoE' not in combined_key:
                            compare_score_dict[combined_key] = (
                                performance_profile_normalized_score_dict[key2],
                                performance_profile_normalized_score_dict[key1])

        # flip the keys in the last dictionary
        # key = 'Impala w/ MoE,Impala'
        # key_old = 'Impala,Impala w/ MoE'
        # compare_score_dict[key] = (compare_score_dict[key_old][1], compare_score_dict[key_old][0])
        # # delte the old key
        # del compare_score_dict[key_old]

        # colors
        colors_probablity = colors_flatten_original[:len(compare_score_dict)]

        # matrices for pairs of algorithms we want to compare
        average_probabilities, average_prob_cis = rly.get_interval_estimates(
            compare_score_dict, metrics.probability_of_improvement, reps=2000)
        axes = plot_utils.plot_probability_of_improvement(
            average_probabilities, average_prob_cis, xlabel=r'P(X $>$ Y)',
            figsize=(2.5, 0.37 * len(compare_score_dict)),
            colors=colors_probablity,
            left_ylabel=r'\textbf{X}',
            right_ylabel=r'\textbf{Y}',
            ticklabelsize='large',
            labelsize='large',
            interval_height=0.6,
            ylabel_x_coordinate=0.3
        )
        plt.savefig(f"{args.output_filename}_probability_of_improvement.pdf", bbox_inches="tight")

    if args.rc.aggregate_metrics_plots:
        print("plotting aggregate metrics")
        aggregate_func = lambda x: np.array([metric_fn(x) for metric_fn in [metric_fns[1]]])  # IQM at [1]
        aggregate_scores, aggregate_score_cis = rly.get_interval_estimates(
            performance_profile_normalized_score_dict, aggregate_func,
            reps=args.rc.interval_estimates_num_bootstrap_reps
        )

        aggregate_scores_df = pd.DataFrame.from_dict(
            aggregate_scores, orient="index", columns=["IQM"]  # , "Median", "IQM", "Mean", "Optimality Gap"]
        )

        # add the upper and lower confidence intervals to the dataframe
        for exp_name in aggregate_scores_df.index:
            aggregate_scores_df.loc[exp_name, "IQM_Lower"] = aggregate_scores_df.loc[exp_name, "IQM"] - \
                                                             aggregate_score_cis[exp_name][0]
            aggregate_scores_df.loc[exp_name, "IQM_Upper"] = aggregate_score_cis[exp_name][1] - \
                                                             aggregate_scores_df.loc[exp_name, "IQM"]

        # All scores
        aggregate_func_all = lambda x: np.array([metric_fn(x) for metric_fn in metric_fns])
        aggregate_scores_all, aggregate_score_cis_all = rly.get_interval_estimates(
            performance_profile_normalized_score_dict, aggregate_func_all,
            reps=args.rc.interval_estimates_num_bootstrap_reps
        )
        aggregate_scores_df_all = pd.DataFrame.from_dict(
            aggregate_scores_all, orient="index", columns=["Median", "IQM", "Mean", "Optimality Gap"]
        )
        print_rich_table(f"Aggregate Scores", aggregate_scores_df_all.reset_index(), console)

        # format this in a single table column by combining the median and upper lower confidence intervals
        save_results = aggregate_scores_df.reset_index()
        save_results["IQM"] = save_results["IQM"].map("{:.2f}".format)
        save_results["IQM_Lower"] = save_results["IQM_Lower"].map("{:.2f}".format)
        save_results["IQM_Upper"] = save_results["IQM_Upper"].map("{:.2f}".format)
        save_results["IQM"] = save_results["IQM"] + " ^{ \pm [" + save_results["IQM_Lower"] + ", " + \
                              save_results["IQM_Upper"] + "]}"
        save_results = save_results.drop(columns=["IQM_Lower", "IQM_Upper"])
        save_results.to_markdown(open(f"{args.output_filename}_aggregate.md", "w"))

        if args.rc.overlay_train_test:
            # calculate the generalization gap and save it as a markdown table just like the aggregate scores
            aggregate_scores_testing_all = aggregate_scores_all
            aggregate_scores_training_all, _ = rly.get_interval_estimates(
                overlay_normalized_score_dict,
                aggregate_func_all,
                reps=args.rc.interval_estimates_num_bootstrap_reps)

            generalization_gap_all = {}
            for exp_name in aggregate_scores_testing_all:
                generalization_gap_all[exp_name] = aggregate_scores_training_all[exp_name + ":train"] - \
                                                   aggregate_scores_testing_all[exp_name]

            generalization_gap_all_df = pd.DataFrame.from_dict(generalization_gap_all, orient="index",
                                                               columns=["Median", "IQM", "Mean", "Optimality Gap"])
            print_rich_table(f"Generalization Gap", generalization_gap_all_df.reset_index(), console)
            # save this generalization gap table as a markdown file
            generalization_gap_all_df.to_markdown(open(f"{args.output_filename}_generalization_gap.md", "w"),
                                                  index=True)

        # result += [f"{metric_result.mean():.2f} ± {metric_result.std():.2f}"]

        fig, axes = plot_utils.plot_interval_estimates(
            # aggregate_scores,
            # aggregate_score_cis,
            # metric_names=["Median", "IQM", "Mean", "Optimality Gap"],
            aggregate_scores,
            aggregate_score_cis,
            metric_names=["IQM"],  # "IQM", "Mean", "Optimality Gap"],
            algorithms=exp_names,
            colors=colors,
            xlabel="",
            fontsize="small",
            row_split_after=[args.pc.row_split_after]
            # xlabel='Normalized Score',
            # xlabel_y_coordinate=-0.08 # this variable needs to be adjusted for each plot... :( so we just disable xlabel for now.
        )
        if args.rc.overlay_train_test and overlay_normalized_score_dict:
            overlay_point_estimates, overlay_interval_estimates = plot_utils.compute_overlay_estimates(
                overlay_normalized_score_dict,
                aggregate_func,
                args.rc.interval_estimates_num_bootstrap_reps,
            )
            plot_utils.overlay_interval_bars(
                axes,
                exp_names,
                overlay_point_estimates,
                overlay_interval_estimates,
                overlay_pairs,
                overlay_colors,
                linewidth=args.rc.overlay_linewidth,
                alpha=args.rc.overlay_alpha,
            )
        if args.rc.normalized_score_threshold_min is not None and args.rc.normalized_score_threshold is not None:
            axes.set_xlim(args.rc.normalized_score_threshold_min - 0.0, args.rc.normalized_score_threshold + 0.0)
        # axes.set_xlim(args.rc.normalized_score_threshold_min - 0.05, args.rc.normalized_score_threshold + 0.05)

        # the ticks on x axis should be from 0 to args.rc.normalized_score_threshold with step size 0.1
        if args.rc.normalized_score_threshold_min is not None:
            # set x min max
            # if dormant_neuron plot then 0.05 otherwise 0.1
            step_size = 0.1
            if 'dormant' in args.output_filename:
                step_size = 0.05

            axes.set_xticks(
                np.arange(
                    args.rc.normalized_score_threshold_min,
                    args.rc.normalized_score_threshold + 0.01,
                    step_size
                )
            )

        # align the yticks labels to the left side
        # for label in axes.get_yticklabels():
        #     label.set_ha('left')

        # axes[1].set_xlabel("Normalized Score", fontsize="large")
        axes.set_xlabel(f"Final {args.pc.ylabel} (IQM)", fontsize="large")
        # add title "Summary of Aggregate Metrics"
        title_name = args.output_filename.split('/')[-1]
        title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'

        if args.rc.aggregate_fig_title is not None:
            title_name = f' {args.rc.aggregate_fig_title}'
            title_name = r'\textbf{' + title_name + '}'

        axes.set_title(f"{title_name}", fontsize="large")

        # fig.tight_layout()
        # plt.savefig(f"{args.output_filename}_aggregate.png", bbox_inches="tight")
        plt.savefig(f"{args.output_filename}_aggregate.pdf", bbox_inches="tight")

if args.report:
    print("saving report")
    report = wb.Report(
        project=args.wandb_project_name,
        title=f"Regression Report: {exp_name}",
        description=str(args.filters),
        blocks=blocks,
    )
    report.save()
    print(f"view the generated report at {report.url}")
